...
void remove_ingress(
    ...
    for (auto& op : curr->ops) {
        ...
        if (have_dep(o, *op)) {
            auto it = l.find(Label::ingress);
            if (it != l.end()) {
                l.erase(Label::ingress);
                delta++;
                break;
            }
        }
    ...
    for (auto& e : curr->branches) {
        ...
        assert(n_bb != nullptr);
        remove_ingress(o, visited, n_bb, delta);
    }
...
void remove_egress(
    ...
    for (auto& op : curr->ops) {
        auto& l = op->meta_ref<LabelSet>();
        if (have_dep(o, *op)) {
            auto it = l.find(Label::egress);
            if (it != l.end()) {
                l.erase(Label::egress);
                delta++;
                break;
            }
        }
    ...
    for (auto& prev_bb : prev_bbs_iter->second) {
        remove_egress(o, from, visited, prev_bb, delta);
    }
}
    ...
    do {
        ...
        for (auto& bb : entry_f->bbs) {
            for (int op_idx = 0; op_idx < bb->ops.size(); op_idx++) {
                ...
                if (labels.find(Label::ingress) == labels.end()) {
                    // ops that depends on this one can not be ingress
                            ...
                            for (auto& u : d->uses) {
                                if (u.type == Var::Use::T::BB_COND) {
                                    ...
                                    for (auto& o : dep_bb_ptr->ops) {
                                        ...
                                        if (ol.find(Label::ingress) != ol.end()) {
                                            ol.erase(Label::ingress);
                                            delta++;
                                        }
                                    }
                                }
                    ...
                    for (auto& e : bb->branches) {
                        auto n_bb = e.next_bb.lock();
                        remove_ingress(*op, visited, n_bb, delta);
                    }
                    if (!bb->is_return && !bb->is_err) {
                        auto n_bb = bb->default_next_bb.lock();
                        assert(n_bb != nullptr);
                ...
                if (labels.find(Label::egress) == labels.end()) {
                    // ops that this one deponds on can not be egress
                    ...
                    for (auto& prev_bb : from[bb]) {
                        remove_egress(*op, from, visited, prev_bb, delta);
                    }
                }
            }
        }
...
PartitionResult partition(const Function& func) {
    ...
    for (auto& bb : result.ingress->bbs) {
    ...
    remove_by_label(
        *result.cpu,
        [](const std::unordered_set<Label>& ls) -> bool {
            auto have_ingress = ls.find(Label::ingress) != ls.end();
            auto have_egress = ls.find(Label::egress) != ls.end();
            return have_ingress || have_egress;
        },
        [](const std::unordered_set<Label>& ls) -> bool {
            auto have_ingress = ls.find(Label::ingress) != ls.end();
            auto have_egress = ls.find(Label::egress) != ls.end();
    ...
    remove_by_label(
        *result.egress,
        [](const std::unordered_set<Label>& ls) -> bool {
            auto have_ingress = ls.find(Label::ingress) != ls.end();
            auto not_egress = ls.find(Label::egress) == ls.end();
            return have_ingress || not_egress;
        },
        [](const std::unordered_set<Label>& ls) -> bool {
            return false;
        });

    remove_empty_bb(*result.ingress);
    remove_empty_bb(*result.cpu);
    remove_empty_bb(*result.egress);

    // now find all the transfered vars
    find_transferred_vars(result.ingress, result.cpu, result.ingress_to_cpu_vars);
    find_transferred_vars(result.ingress, result.egress, result.cpu_to_egress_vars);
    find_transferred_vars(result.cpu, result.egress, result.cpu_to_egress_vars);
    return result;
}
}
